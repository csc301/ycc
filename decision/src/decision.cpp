...
int given_height=100,hp =80,hd = 60,hi=100,removei=60,haha=3;
...
void land_callback(const std_msgs::Bool msg)
{
        land_flag = msg.data;
}
...
int main(int argc,char **argv)
{
    ...
    ros::Subscriber  land_flag_sub = n.subscribe("land_flag",10,land_callback);
    ...
    while(ros::ok())
    {
       hei_error = given_height*0.01-height;
       ...
       if(out_z<-90)out_z=-90;
       //if(out_z<0)out_z-=20;
       if(land_flag)
       {
                avo_msg.linear.z = LAND_THRUST;
                tracker_msg.linear.z = LAND_THRUST;
                hover_msg.linear.z = LAND_THRUST;
       }
       else
       {
